...
contract MiniMatch is Ownable {
    ...
    function createMatches(
        ...
        uint256 _idx;
        require(
            _descriptions.length == _startTimes.length &&
    ...
    function updateMatchScores(
        ...
        require(_matchIds.length > 0, "size = 0");
        require(
            _matchIds.length == _point1.length &&
                _point1.length == _point2.length,
            "not-same-size"
        );
        ...
        for (uint256 i = 0; i < _matchIds.length; i++) {
            updateMatchScore(
                _matchIds[i],
                MDataTypes.Score(_point1[i], _point2[i])
            );
        }
    }
    ...
    function updateMatchStatuses(
        ...
        MDataTypes.MatchStatus[] memory _status
    ) external onlyOwnerOrAdmin {
        require(_matchIds.length > 0, "size = 0");
    ...
    function info(uint256 _matchId)
        external
        view
        returns (MDataTypes.Match memory _match)
    {
    ...
    function updateMatchScore(uint256 _matchId, MDataTypes.Score memory _score)
        internal
    {
    ...
    function updateMatchStatus(uint256 _matchId, MDataTypes.MatchStatus _status)
        internal
    {
        MDataTypes.Match storage _match = matches[_matchId];
    ...
    event MatchScoreUpdated(
        address caller,
        uint256 matchId,
        MDataTypes.Score score
    );
    event MatchStatusUpdated(
        address caller,
        uint256 matchId,
        MDataTypes.MatchStatus status
    );
    event MatchCreated(
        uint256 idx,
