...
contract SponsoredPredictionLink is Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(address => mapping(uint256 => SDataTypesExt.SPrediction)))
        public predictions;
    mapping(address => mapping(uint256 => SDataTypesExt.SPredictStats))
    ...
    function setSponsorTotal(uint256 _matchId, uint256 _amount)
        public
        onlyOwner
    {
    ...
    function setRewardTotal(uint256 _matchId, uint256 _amount)
        public
        onlyOwner
    {
    ...
    function predict(
        ...
        require(
            _match.status == SDataTypesExt.MatchStatus.AVAILABLE,
            "match-not-available"
        );
        ...
        )
    {
        ...
        uint256 _totalPredict = predictStats[_token][_matchId].predictionAmount[
            _winSide
        ];
        _reward = 0;
        _sponsorReward = 0;
        ...
        if (_totalPredict == 0) {
            for (
                uint256 i = 0;
                i <
                predictions[_token][_user][_matchId].predictionAmount.length;
                i++
            ) {
                _reward += predictions[_token][_user][_matchId]
                    .predictionAmount[i];
            }
        } else {
            uint256 _total = 0;
            uint256 _userPredict = predictions[_token][_user][_matchId]
                .predictionAmount[_winSide];

            for (
                uint256 i = 0;
                i < predictStats[_token][_matchId].predictionAmount.length;
                i++
            ) {
    ...
    function getMatchInfo(uint256 matchId, address token)
        public
        view
        returns (SDataTypesExt.SPredictStats memory)
    {
    ...
    function claimReward(uint256 _matchId, address[] memory _tokens) external {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            if (
                predictions[_tokens[i]][msg.sender][_matchId].claimed == false
            ) {
                (_reward, _sponsorReward, _rewardAmount) = calculateReward(
                    _matchId,
                    msg.sender,
                    _tokens[i]
                );
