...
contract SponsoredMatchLink is Ownable {
    ...
    function setAdminPermission(address _admin, bool _status)
        external
        onlyOwner
    {
    ...
    function updateSToken(uint256 _matchId, address _sToken)
        external
        onlyOwnerOrAdmin
    {
    ...
    function updateOToken(uint256 _matchId, address _oToken)
        external
        onlyOwnerOrAdmin
    {
    ...
    function updateMatchScoresManual(uint256 _matchId, uint256 _wOption)
        external
        onlyOwnerOrAdmin
    {
        require(
            block.timestamp > matches[_matchId].resultTime,
            "invalid-end-time"
        );
        require(
            matches[_matchId].status == SDataTypesExt.MatchStatus.AVAILABLE,
            "match-not-available"
        );
    ...
    function updateMatchScores(uint256 _matchId) external {
        ...
        for (uint256 i = 0; i < _sResult.rType.length; i++) {
            if (_sResult.rType[i] == 0) {
                // range from to
                if (
                    tPrice > _sResult.rValueFrom[i] * 10**5 &&
                    tPrice < _sResult.rValueTo[i] * 10**5
                ) {
                    // won
    ...
    function updateMatchStatuses(
        ...
        SDataTypesExt.MatchStatus[] memory _status
    ) external onlyOwnerOrAdmin {
    ...
    function info(uint256 _matchId)
        ...
        view
        returns (
            SDataTypesExt.Match memory _match,
            SDataTypesExt.FinalResult memory _finalPrice
        )
    {
    ...
    function updateMatchStatus(
        uint256 _matchId,
        SDataTypesExt.MatchStatus _status
    ) internal {
        SDataTypesExt.Match storage _match = matches[_matchId];
    ...
    event MatchStatusUpdated(
        address caller,
        uint256 matchId,
        SDataTypesExt.MatchStatus status
    );
    event MatchCreated(
        uint256 idx,
